class Train
  ...
  # Все, что выше используется другими частями программы.
  # Все, что ниже используется только внутри класса. Но при добавлении
  # функции "подскажите, какая следующая станция" придется обращаться к
  # этим методам из вне. Поэтому прятать не стал.
